...
void main(void)
{
	...
	// emission
	emission = front.emission * image_color;
	...
	for(int i = 0; i < light_count; i++) // for all light sources
	{
		...
		// ambient
		ambient = lights[i].ambient * front.ambient * image_color;
		...
		diffuse = atten * lights[i].diffuse * front.diffuse * image_color * vec4(vec3(angle),1.0);
