...
make -j4 -C external/Galaxy/components/console/build/Shared/Debug --no-print-directory
if [ $? -ne 0 ]; then
	exit 1
fi

cmake -Hcomponents/core -Bcomponents/core/build/Shared/Debug
make -j4 -C components/core/build/Shared/Debug --no-print-directory
if [ $? -ne 0 ]; then
	exit 1
fi
...
make -j4 -C components/physx/build/Shared/Debug --no-print-directory
if [ $? -ne 0 ]; then
	exit 1
fi
...
make -j4 -C components/final/build/Shared/Debug --no-print-directory
if [ $? -ne 0 ]; then
	exit 1
fi

cmake -Hcomponents/ext -Bcomponents/ext/build/Shared/Debug
make -j4 -C components/ext/build/Shared/Debug --no-print-directory
if [ $? -ne 0 ]; then
	exit 1
fi
